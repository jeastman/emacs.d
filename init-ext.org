...
| =ido-use-filename-at-point=          | =t=        | Attempt to use filename at point as starting point |
| =ido-use-url-at-point=               | =t=        | Attempt to use URL at point                        |
...
positioning stuff over more to the right and adding the
=global-mode-string= into its own section. When nothing is set in that
portion, it remains empty. As of now, I manually turn on battery status if
needed.
