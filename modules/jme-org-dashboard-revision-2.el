...
(defun jme-org-dashboard--parse-org-data ()
  ...
  (let* (;; THE DEFINITIVE FIX IS HERE:
         ;; By binding `org-element-use-cache' to nil, we force a fresh,
         ;; complete parse that correctly associates property drawers with their items.
         (org-element-use-cache nil)
         (org-tree (org-element-parse-buffer nil t))
...
(defun jme-org-dashboard--parse-org-data-old ()
  "Parse the current Org buffer and return a plist of dashboard data."
  ;; THE FIX IS HERE: Change (org-element-parse-buffer) to (org-element-parse-buffer nil t)
  ;; The `nil` means parse all element types.
  ;; The `t` for AFFILIATION is crucial: it tells Org to create :parent links.
  (let* ((org-tree (org-element-parse-buffer nil t))
         (doc-props (jme-org-dashboard--get-document-properties org-tree))
         (categories (jme-org-dashboard--get-categories-and-links org-tree)))
    (list :title (or (plist-get doc-props :title) "Dashboard")
          :description (plist-get doc-props :description)
...
(defun jme-org-dashboard--parse-org-data-old-old ()
  "Parse the current Org buffer and return a plist of dashboard data."
...
(defun jme-org-dashboard--get-document-properties (org-tree)
  ...
  (let (props)
    ;; THE FIX: The lambda must take exactly ONE argument.
    (org-element-map org-tree 'keyword
      (lambda (kw)
        (let ((key-str (plist-get kw :key)))
          (if key-str
              (let* ((key (intern (concat ":" (downcase key-str))))
                     (val (plist-get kw :value)))
                (setq props (plist-put props key val))))))
      nil) ; Explicitly pass nil for the optional INFO argument.
    props))

(defun jme-org-dashboard--get-document-properties-old (org-tree)
  "Extract #+KEY: VALUE properties from the parsed ORG-TREE."
  (let (props)
    ;; CORRECTED: Use a 2-argument lambda to ignore the INFO plist.
    (org-element-map org-tree 'keyword
      (lambda (kw _info)
        ;; REPLACED `when-let` with standard `let` and `if` for portability.
        (let ((key-str (plist-get kw :key)))
          (if key-str
              (let* ((key (intern (concat ":" (downcase key-str))))
                     (val (plist-get kw :value)))
...
(defun jme-org-dashboard--get-document-properties-old-old (org-tree)
  "Extract #+KEY: VALUE properties from the parsed ORG-TREE."
  (let (props)
    ;; CORRECTED: Use a 2-argument lambda to ignore the INFO plist.
    (org-element-map org-tree 'keyword
...
(defun jme-org-dashboard--get-document-properties-old-old-old (org-tree)
  "Extract #+KEY: VALUE properties from the parsed ORG-TREE.
...
(defun jme-org-dashboard--get-properties-for-link-to-be-deleted (link-element)
  "Find the property plist for a LINK-ELEMENT by navigating up the AST."
  (message "--> 0 <--")
  ; (message "LINK-ELEMENT: %s" link-element)
  (let (properties-plist)
    (let* ((parent (org-element-property :parent link-element)))
      (message "--> 1 <--")
      (if parent
          (let ((item (org-element-property :parent parent)))
            (message "--> 2 <--")
            (if (and item (eq (org-element-type item) 'item))
                ;; THE FIX: Pass nil for INFO. `first-match` doesn't use a lambda,
                ;; so it's unaffected, but being explicit is better.
                (let ((drawer (org-element-map (org-element-contents item) 'property-drawer 'first-match nil)))
                  (message "--> 3 <--")
                  (message "DRAWER: %s" drawer)
                  (if drawer
                      (dolist (node-prop (org-element-contents drawer))
                        (if (eq (org-element-type node-prop) 'node-property)
...
(defun jme-org-dashboard--get-properties-for-link-old (link-element)
  "Find the property plist for a LINK-ELEMENT by navigating up the AST."
  (let (properties-plist)
    ;; REPLACED `when-let*` with standard nested `let` and `if`.
    (let* ((parent (org-element-property :parent link-element)))
      (if parent
          (let ((item (org-element-property :parent parent)))
            (if (and item (eq (org-element-type item) 'item))
                (let ((drawer (org-element-map (org-element-contents item) 'property-drawer 'first-match nil))) ; Use nil for INFO
                  (if drawer
                      (dolist (node-prop (org-element-contents drawer))
                        (if (eq (org-element-type node-prop) 'node-property)
                            (let ((key-str (org-element-property :key node-prop)))
...
(defun jme-org-dashboard--get-properties-for-link-old-old (link-element)
  "Find the property plist for a LINK-ELEMENT by navigating up the AST.
This now works because the AST has :parent links."
  (message "--> 0 <--")
  (message "LINK-ELEMENT: %s" link-element)
  (let (properties-plist)
    ;; This logic is correct and will now succeed.
    ;; The link's parent is a paragraph, whose parent is the item.
...
(defun jme-org-dashboard--extract-links-from-headline (headline)
  "Extract links and their properties from a headline's section.
This version uses a robust item-first parsing strategy with CATCH/THROW."
  (let (links)
    ;; Find all list `item' elements within this headline's section.
    (org-element-map (org-element-contents headline) 'item
      (lambda (item)
        ;; For each item, find the link and the property drawer within it.
        ;; We use the standard `catch`/`throw` idiom for maximum compatibility.
        (let* ((link-element
                (catch 'found-link
                  (org-element-map item 'link
                    (lambda (link) (progn (message "FOUND LINK")(throw 'found-link link))))
                  nil)) ; Return nil if nothing is thrown.
               (drawer
                (catch 'found-drawer
                  (org-element-map item 'drawer
                    (lambda (dr) (progn (message "FOUND DRAWER")(throw 'found-drawer dr))))
                  nil)) ; Return nil if nothing is thrown.
               (props nil))

          ;; Only proceed if we actually found a link in this list item.
          (when link-element
            (message "--> link-element <--")
            ; (message "LINK-ELEMENT: %s" link-element)
            ;; If we also found a property drawer, parse its contents into a plist.
            (when drawer
              (message "--> drawer <--")
              (message "DRAWER: %s" drawer)
              (dolist (node-prop (org-element-contents drawer))
                (when (eq (org-element-type node-prop) 'node-property)
                  (message "--> node-property <--")
                  (let ((key-str (org-element-property :key node-prop)))
                    (if key-str
                        (setq props
                              (plist-put props
            ...
            ;; Now, build the final link data structure using what we found.
            (let* ((url (org-element-property :raw-link link-element))
                   (label (or (car (org-element-contents link-element)) url))
    ...
    ;; Sort the collected links according to the user's preference.
...
(defun jme-org-dashboard--extract-links-from-headline-old (headline)
  "Extract links and their properties from a HEADLINE's section.
This version uses a robust item-first parsing strategy."
  (let (links)
    ;; Find all list `item' elements within this headline's section.
    (org-element-map (org-element-contents headline) 'item
      (lambda (item)
        ;; For each item, find the link and the property drawer within it.
        ;; `first-match' is efficient as an item should only have one of each.
        (let* ((link-element (org-element-map item 'link 'first-match nil))
               (drawer (org-element-map item 'property-drawer 'first-match nil))
               (props nil))
...
(defun jme-org-dashboard--extract-links-from-headline-old-old (headline)
  ...
  (let (links)
    ...
    (org-element-map (org-element-contents headline) 'link
      (lambda (link)
        (let* ((props (jme-org-dashboard--get-properties-for-link link))
          ...
          (push (list :url url :label label :icon icon :note note :tags tags) links)))
      nil) ; Explicitly pass nil for INFO.
    (let ((sorted-links (nreverse links)))
       (if (eq jme-org-dashboard-sort-order 'alphabetical)
           (sort sorted-links (lambda (a b) (string< (plist-get a :label) (plist-get b :label))))
         sorted-links))))

(defun jme-org-dashboard--extract-links-from-headline-old-old-old (headline)
  "Extract links and their associated properties from a headline's section."
  (let (links)
    ;; CORRECTED: Use a 2-argument lambda to get the actual LINK element.
    (org-element-map (org-element-contents headline) 'link
      (lambda (link _info)
        (let* ((props (jme-org-dashboard--get-properties-for-link link))
               (url (org-element-property :raw-link link))
               ...
               (icon (plist-get props :ICON))
               (note (plist-get props :NOTE))
               ;; REPLACED `when-let` with standard `let` and `if`.
               (tags (let ((tags-str (plist-get props :TAGS)))
                       (if tags-str
                           (split-string tags-str "," t " ")))))
          (push (list :url url :label label :icon icon :note note :tags tags) links))))
...
(defun jme-org-dashboard--extract-links-from-headline-old-old-old-old (headline)
  "Extract links and their associated properties from a HEADLINE's section."
  (let (links)
    ;; This mapping finds all links within the headline section.
    (org-element-map (org-element-contents headline) 'link
      (lambda (link _info)
        (let* ((props (jme-org-dashboard--get-properties-for-link link))
               (url (org-element-property :raw-link link))
